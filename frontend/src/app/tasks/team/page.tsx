...
export default function TeamTasksPage() {
  ...
  type SortBy = 'due_asc' | 'due_desc' | 'status' | 'assignee';
  const [statusFilter, setStatusFilter] = useState<StatusFilter>('all');
  const [sortBy, setSortBy] = useState<SortBy>('due_asc');
  const token = storage.getToken();

  // Role-based access: Manager+ can view team tasks
  ...
  const canView = getRank(user?.role) >= 2; // Manager+

  useEffect(() => {
    const load = async () => {
      if (!canView) {
        setLoading(false);
        setError('You do not have permission to view team tasks.');
        return;
      }
      if (!user?.teamId || !token) return;
      try {
        setLoading(true);
        const res = await taskService.getTasksByTeam(token, String(user.teamId));
        if (res.status === 'success') setTasks(res.data);
        else setError(res.message || 'Failed to fetch team tasks');
      } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="p-6 bg-white rounded shadow text-center max-w-md">
          ...
          <p className="text-gray-600 text-sm">Only Managers and above can view team tasks.</p>
        </div>
      </div>
    );
  }
      ...
      <main>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500">Showing tasks for your team</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500 bg-white rounded-lg shadow">No team tasks found.</div>
          ) : (
            <div>
              {/* Controls */}
